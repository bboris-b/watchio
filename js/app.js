...
const firebaseConfig = {
    apiKey: "AIzaSyD0yAOVQsfJqY68K4oIRTm3ai-5j4tteQk",
    authDomain: "ai-transcript-analytics.firebaseapp.com",
    projectId: "ai-transcript-analytics",
    storageBucket: "ai-transcript-analytics.firebasestorage.app",
    messagingSenderId: "520410492515",
    appId: "1:520410492515:web:9110b46f08c46d360e1256"
  };
...
class FirebaseTracker {
    ...
    async init() {
        if (typeof firebase !== 'undefined' && window.analyticsEnabled) {
            try {
    ...
    async track(provider, success, responseTime, complexity, error = null) {
        if (!this.db || !window.analyticsEnabled) return;
...
let firebaseTracker = new FirebaseTracker();
...
// Add this code to your main AI Meeting Tool JavaScript (app.js)

// Provider Analytics Tracker Class
class ProviderTracker {
    ...
    init() {
        // Try to connect to dashboard if it's already open
        this.findDashboardWindow();
    }

    findDashboardWindow() {
        // Check if dashboard is open in another tab/window
        try {
            if (window.opener && window.opener.location.href.includes('dashboard')) {
                this.dashboardWindow = window.opener;
            }
        } catch (e) {
            // Cross-origin or dashboard not found
        }
    }

    openDashboard() {
        // Open dashboard in new window
        const dashboardUrl = './dashboard.html'; // Adjust path as needed
    ...
    trackProviderCall(provider, success, responseTime, complexity, error = null) {
        const callData = {
            type: 'provider_call',
            provider: provider,
            success: success,
        ...
        if (this.dashboardWindow && !this.dashboardWindow.closed) {
            try {
                this.dashboardWindow.postMessage(callData, '*');
            } catch (e) {
                console.log('Could not send to dashboard:', e);
            }
        }

        // Also store locally for dashboard to pick up later
        this.storeLocalTracking(callData);
    }

    storeLocalTracking(callData) {
        try {
            const existing = JSON.parse(localStorage.getItem('provider_analytics') || '{"calls":[]}');
            existing.calls = existing.calls || [];
            existing.calls.push(callData);
            
            // Keep only last 1000 calls
            if (existing.calls.length > 1000) {
                existing.calls = existing.calls.slice(-1000);
            }
            
            localStorage.setItem('provider_analytics', JSON.stringify(existing));
        } catch (e) {
            console.log('Could not store tracking data:', e);
        }
    }
}
...
// MODIFY THE SmartProviderManager CLASS
// Add this to your existing SmartProviderManager class:

// In the generateSummary method, add tracking before the return:
/*
OLD:
return {
    summary: result,
...
providerTracker.trackProviderCall(
    provider.name, 
    true, 
...
// In each provider call method, add timing and tracking:
...
async callOllama(prompt) {
    ...
    try {
        ...
        if (!response.ok) {
            const error = `Ollama error: ${response.status}`;
            providerTracker.trackProviderCall('ollama_local', false, Date.now() - startTime, 'unknown', error);
            throw new Error(error);
        }
        
        const data = await response.json();
        providerTracker.trackProviderCall('ollama_local', true, Date.now() - startTime, 'medium');
        return data.response;
    } catch (error) {
        providerTracker.trackProviderCall('ollama_local', false, Date.now() - startTime, 'unknown', error.message);
        throw error;
    }
}
...
async callGemini(prompt) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            providerTracker.trackProviderCall('gemini_free', false, Date.now() - startTime, 'medium', error);
            throw new Error(error);
        }
        
        const data = await response.json();
        providerTracker.trackProviderCall('gemini_free', true, Date.now() - startTime, 'medium');
        return data.candidates[0].content.parts[0].text;
    } catch (error) {
        providerTracker.trackProviderCall('gemini_free', false, Date.now() - startTime, 'unknown', error.message);
        throw error;
    }
}
...
async callHuggingFace(prompt) {
    const startTime = Date.now();
    const HF_TOKEN = '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'; // Configure this
    
    if (HF_TOKEN === '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') {
        const error = 'HuggingFace token not configured';
        providerTracker.trackProviderCall('huggingface', false, 0, 'unknown', error);
        throw new Error(error);
    }
    ...
    try {
        ...
        if (!response.ok) {
            const error = `HuggingFace error: ${response.status}`;
            if (response.status === 429) {
                this.rateLimits.set('huggingface', Date.now() + 1800000);
            }
            providerTracker.trackProviderCall('huggingface', false, Date.now() - startTime, 'complex', error);
            throw new Error(error);
        }
        
        const data = await response.json();
        providerTracker.trackProviderCall('huggingface', true, Date.now() - startTime, 'complex');
        return data[0].generated_text;
    } catch (error) {
        providerTracker.trackProviderCall('huggingface', false, Date.now() - startTime, 'unknown', error.message);
        throw error;
    }
}
...
generateEnhancedTemplate(transcript, notes, meetingType, complexity) {
    ...
    // Your existing generateEnhancedTemplate code here...
    const result = `Your enhanced template result...`;
    
    // Track the call
    providerTracker.trackProviderCall('enhanced_template', true, Date.now() - startTime, complexity.level);
    
    return result;
}

// ADD TO THE MAIN APP UI
// Add dashboard button to your main interface (in the controls section):

/*
Add this HTML to your main interface:
<button onclick="openDashboard()" class="btn btn-secondary" title="Open Analytics Dashboard">
    📊 Analytics
</button>
*/
...
function openDashboard() {
    providerTracker.openDashboard();
    showMessage('Analytics dashboard opened in new window', 'info');
}

// EXAMPLE INTEGRATION IN generateSummary function:
/*
async function generateSummary() {
    if (transcript.length === 0) {
        showMessage('No transcript available for summary generation', 'error');
...
console.log('Provider tracking integration loaded successfully!');
console.log('Use providerTracker.openDashboard() to open analytics dashboard');